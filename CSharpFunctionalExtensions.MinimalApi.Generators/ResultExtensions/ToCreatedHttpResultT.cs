...
public partial class ResultExtensionsGenerator
{
    ...
    {
        return $$"""
               public static {{httpResultType}} ToCreatedHttpResult<T>(this Result<T,{{resultErrorType}}> result, Uri? uri = null, int failureStatusCode = 400)
               {
               ...
               public static async Task<{{httpResultType}}> ToCreatedHttpResult<T>(this Task<Result<T,{{resultErrorType}}>> result, Uri? uri = null, int failureStatusCode = 400)
               {
