...
public static partial class ResultExtensions
{
    ...
    /// </summary>
    public static Results<Accepted<T>, ProblemHttpResult> ToAcceptedHttpResult<T>(this Result<T> result, Func<T, Uri> uri, int failureStatusCode = 400)
    {
    ...
    /// Returns a <see cref="Accepted{TValue}"/> with Accepted status code in case of success result. Returns <see cref="ProblemHttpResult"/> in case of failure. You can provide an URI to create a location HTTP-Header. You can override the error status code.
    /// </summary>
    public static async Task<Results<Accepted<T>, ProblemHttpResult>> ToAcceptedHttpResult<T>(this Task<Result<T>> result, Func<T, Uri> uri, int failureStatusCode = 400)
    {
