...
public static class FindBookEndpoint
{
  ...
  private static Results<Ok<Book>, ProblemHttpResult> Handle(BookService service, Guid id) => Maybe.From(service.Find(id)).ToResult($"Couldn't find book with id {id}.").ToOkHttpResult();
}